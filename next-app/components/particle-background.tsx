...
export function ParticleBackground() {
  ...
  const PARTICLE_COLOR = { r: 254, g: 223, b: 22 }
  const SPREAD = 20
  const SPEED = 0.1
  const BASE_SIZE = 100
  const PIXEL_RATIO = 1
  ...
  const initParticles = useCallback(
    (width: number, height: number) => {
      const count = Math.floor(
        (width * height) / (BASE_SIZE * SPREAD * 50)
      )
  ...
  }, [initParticles, PIXEL_RATIO, SPEED, PARTICLE_COLOR.r, PARTICLE_COLOR.g, PARTICLE_COLOR.b])
